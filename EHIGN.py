...
class DTIPredictor(nn.Module):  
    def __init__(self, node_feat_size, edge_feat_size, hidden_feat_size, layer_num=3):  
        ...
        for _ in range(layer_num):  
            ...
            # 新增：处理蛋白质内部非共价相互作用的卷积层  
            convpp = NIGConv(hidden_feat_size, hidden_feat_size, feat_drop=0.1)  
        ...
        self.lin_edge_pp_noncov = nn.Linear(11, hidden_feat_size) 
    ...
    def forward(self, bg):
        ...
        # 新增：蛋白质内部非共价相互作用边特征处理  
        ('pocket', 'intra_p_noncov', 'pocket'):self.lin_edge_pp_noncov(bond_feats[('pocket', 'intra_p_noncov', 'pocket')])  
        }
